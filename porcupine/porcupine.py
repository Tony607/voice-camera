...
class Porcupine(object):
    ...
    _OS_SUFFIX_KEYWORD_FILES = {
        OS_TYPE.RASPBERRYPI: '_raspberrypi',
        OS_TYPE.LINUX: '_linux',
        OS_TYPE.WINDOWS: '_windows',
        OS_TYPE.MAC: '_mac',
    }
    ...
    def __init__(
        ...
        scenario. This parameter is used only when 'keywords' is not set.
        :param keywords: List of absolute paths to keywords. Intended to be used for multiple keyword
        ...
        :param tiny: run Porcupine's tiny variant, use less resource with slightly reduced accuracy, it uses tiny 
        keyword file(s) along with the tiny model file,
        default to False.
        """
        if library_path is None:
            library_path = self._default_library_path()
        self.os_type = self._infer_os(library_path)
        
        if keywords is not None and len(keywords) >= 0:
            keyword_file_paths = self._get_keyword_file_paths(keywords, self.os_type, tiny is True)
        if model_file_path is None:
            if tiny is True:
                model_file_path = os.path.join(os.path.dirname(__file__), './lib/common/porcupine_tiny_params.pv')
            else:
    ...
    @classmethod
    def _infer_os(cls, library_path):
        if library_path is None:
            return cls.OS_TYPE.UNKNOWN
        elif 'raspberry-pi' in library_path:
            return cls.OS_TYPE.RASPBERRYPI
        elif 'linux' in library_path:
            return cls.OS_TYPE.LINUX
        elif 'mac' in library_path:
            return cls.OS_TYPE.MAC
        elif 'windows' in library_path:
            return cls.OS_TYPE.WINDOWS
        else:
            return cls.OS_TYPE.UNKNOWN
    @classmethod
    def _get_keyword_file_by_text(cls, keyword, os_type, tiny = False):
        if os_type is cls.OS_TYPE.UNKNOWN:
            return None
        keyword = keyword.lower()
        fname = keyword + cls._OS_SUFFIX_KEYWORD_FILES.get(os_type)
        if tiny is True:
            fname = fname + '_tiny'
        fname = fname + '.ppn'
        fname = os.path.join(os.path.dirname(__file__), './resources/keyword_files', fname)
        ...
        return None
    @classmethod
    def _get_keyword_file_paths(cls, keywords, os_type, tiny = False):
        """get keywords paths from a list of keywords text"""
        keyword_file_paths = []
        for keyword in keywords:
            keyword_file = cls._get_keyword_file_by_text(keyword, os_type, tiny)
            if keyword_file is not None:
                keyword_file_paths.append(keyword_file)
        return keyword_file_paths
    @classmethod
    def _default_library_path(cls):
        ...
        if system == 'Darwin':
            ...
            elif os.uname()[1] == 'raspberrypi' and machine == 'armv7l': # Raspberry Pi
                with open('/sys/firmware/devicetree/base/model', 'r') as f:
                    first_line = f.readline()
                if 'Pi 3' in first_line:
                    return os.path.join(os.path.dirname(__file__), './lib/raspberry-pi/cortex-a53/libpv_porcupine.so')
            else:
